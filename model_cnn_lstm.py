...
class cnn_1d(nn.Module):
    def __init__(self,n_channel):
        ...
        self.conv = nn.Sequential(
                    nn.Conv1d(n_channel, 32, 3, 1, 1, bias=False),#batch*3*1019), 
                    nn.BatchNorm1d(32),
...
class bi_lstm(nn.Module):
    def __init__(self, input_size=32, hidden_size=32, num_layers=2, bidir=True):
        ...
        self.output = torch.nn.Linear(2*hidden_size, 5)
    def forward(self, x):
        batch, seq_len, nums_fea = x.size()
        features, _ = self.lstm1(x)
        output = self.classifier(features)
        output = self.output(output.view(batch * seq_len, -1))
        # x = (x[:, -1, :])#(batch, seq, feature=num_directions * hidden_size)
        return output

class cnn_lstm(nn.Module):# cnn -> lstm -> out
    def __init__(self, n_channel):
        
        super(cnn_lstm, self).__init__()
        self.cnn = cnn_1d(n_channel)
        self.lstm = bi_lstm()
...
if __name__ == '__main__':
    ...
    if TEST:
        # vis
        net = restore_net(save_path+'epoch_114.ckpt')
        net.eval()
        ...
        for i, idx in enumerate([20, 60,
            ...
            print("ACC: {:.4f}".format(right/total))
            print(get_charateristic(predict))
            print(get_charateristic(label))
    ...
    if PAPER_TEST:
        ...
        if SAVE_TEST_FIG:
            ...
            for i in range(len(ecg_val_db)):
                ...
                # 得到p-end, QRS onset end , T-middle
                error_array[i] = caculate_error(get_charateristic(label), get_charateristic(predict))
                right_point_num += right_point(get_charateristic(label),
                                               get_charateristic(predict), 0.016)
            print("ACC: {:.4f}".format(right / total))
